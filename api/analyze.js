...
Vælg de ${maxItems} dækninger der giver STØRST værdi i salgssituationen!`;
    ...
    if (parsed.coverage && Array.isArray(parsed.coverage)) {
      parsed.coverage = parsed.coverage.map(item => {
        ...
        // Create normalized item - handle ALL known field name variants
        const normalized = {
          category: item.category || item.name || item.punkt || 'Ukendt dækning',
          status_a: item.status_a || item.a_status || (item.a ? 'yes' : 'inib'),
          status_b: item.status_b || item.b_status || (item.b ? 'yes' : 'inib'),
          ...
          reason: item.reason || item.details || item.note || autoReason || 'Ingen yderligere detaljer',
